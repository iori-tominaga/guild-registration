...
| 8.1.3 | 2026-05-18 | 【バグ修正】① board.html のギルドカードが `promotedCls` を反映していなかった（基本形のアイコン・ラベル・ステータスをそのまま表示）。`activeCls = promotedCls ?? cls` でアイコン・ラベル・ステータスを進化形に切替、`[しんか済]` バッジを追加。② battle.html のプレイヤースプライトが進化済みでも区別がなかった。進化済みキャラのスプライトに金色オーラ（drop-shadow パルスアニメーション）を表示。スプライトの形は基本形のまま（進化形のピクセルアートは未実装のため）。 |
| 9.0.0 | 2026-05-25 | Quest 09：portals.html（古代の門）を新規追加。GitHub MCP を「ひらかれし 門」として接続済み表示し、Atlassian / Slack / Filesystem / Fetch を「ねむれる 門」として灰色表示。古代石造遺跡風レイアウト（石アーチ + 金色ルーン）。board.html に「⛩ ぼうけんを ほうこくする」ボタン＋モーダルを追加し、現在のキャラクター情報を反映した MCP プロンプトを表示・コピー可能。「冒険の場所」に「こだいの もん」リンクを追加。 |
| 9.1.0 | 2026-05-25 | sprite-lab.html（スプライト解像度くらべ・実験室ページ）を追加。キャラデザイン一新プロジェクトの解像度決定用プロトタイプ。「せんし」を 12×16 / 16×24 / 24×36 / 32×48 の4解像度で並べて比較表示。本番からはリンクされない一時ページ。 |
| 9.2.0 | 2026-08-30 | 本アプリを `oshou35/tommy-games` のモノレポへ集約するための移行キットを `migration/` に追加。`migrate-to-tommy-games.sh`（履歴を `games/<name>/` へ書き換えて取り込むスクリプト）、全ゲーム共通の GitHub Pages ワークフロー、ゲームランチャー、モノレポ用 CLAUDE.md を同梱。アプリ本体（`src/`）は無変更。集約後の公開URLは `https://oshou35.github.io/tommy-games/guild-registration/` になる |
