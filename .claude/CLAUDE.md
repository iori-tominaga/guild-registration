# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## プロジェクト概要

ドラゴンクエスト風UIの冒険者ギルドウェブアプリ。GitHub Pages で公開される静的HTMLアプリ。ビルドツール不要。

**公開URL**: https://iori-tominaga.github.io/guild-registration/

---

@rules/world.md

---

@rules/deploy-checklist.md

---

@rules/architecture.md

---

@rules/monsters.md

---

@game-spec/world.md

---

@game-spec/mechanics.md

---

## バージョン管理ルール

アプリのバージョンは **`ver. X.Y.Z`** 形式で管理する。

| 桁 | 増やすタイミング |
|---|---|
| **X**（左） | クエストによるコード更新 |
| **Y**（中） | クエスト以外の通常アップデート |
| **Z**（右） | バグ修正 |

**リセットルール：**
- X を上げたら → Y・Z を 0 にリセット
- Y を上げたら → Z を 0 にリセット

バージョンはコミットメッセージ・ドキュメントの変更履歴に必ず記載すること。

---

## 指示確認ルール

オーナーから新しい指示・要件を受け取ったときは、**実装前に必ず理解内容をオーナーに言葉で確認する**。

手順：
1. 指示を自分の言葉で噛み砕いて説明する
2. オーナーに「この理解で合っていますか？」と確認する
3. 承認を得てから実装を開始する

複雑な指示ほど認識のズレが起きやすい。手戻りを防ぐために省略しないこと。

---

## Claude 学習サポートルール

このプロジェクトはオーナーが **Claude Code の使い方を学ぶ**ために進めている。
そのため、各クエストの作業説明には必ず以下の2項目を含めること。

### 1. 今回使った Claude の機能
クエストで実際に使った機能を列挙し、それぞれ一言で何のために使ったか説明する。

例）
| 使った機能 | 用途 |
|---|---|
| `/init` スキル | CLAUDE.md の初期生成 |
| `Edit` ツール（複数回） | 既存ファイルを部分的に書き換え |
| `Agent` サブエージェント | 大きなファイル生成をタイムアウトなく実行 |
| `TodoWrite` ツール | 作業ステップの進捗管理 |

### 2. より活用できる Claude の機能（提案）
今回やらなかったが、使いこなせばより効率的・高品質になる機能を提案する。
「なぜ良くなるか」をセットで説明すること。

例）
- **`/review` スキル** — 実装後にコードレビューをかけると、バグや改善点を自動で指摘してもらえる
- **`/ultrareview`** — PR 単位で多角的なレビューが受けられる。チームレビューの代替になる
- **並列 Agent 実行** — 複数ファイルの作成を同時に走らせると実装時間を短縮できる

---

## ドキュメント更新ルール

**コードを変更したら、必ず以下のドキュメントも同じコミットで更新すること。**

| ドキュメント | 更新が必要なとき |
|---|---|
| `docs/specs/requirements.md` | 機能追加・変更・削除をしたとき |
| `docs/specs/external-spec.md` | 新しいページ・機能のテスト仕様を追加するとき |

更新の手順：
1. `requirements.md` のバージョンを上げ、変更内容を「7. 変更履歴」に追記する
2. `external-spec.md` のバージョンを合わせ、新機能に対応するテストケース（TC-XX）を追加する
3. コードとドキュメントを同じコミットに含める

---

## デプロイ

`master` ブランチに push すると GitHub Actions が自動デプロイ（約1〜2分）。

```
変更 → commit → push origin master → 自動デプロイ → 公開URL反映
```

---

## コミットメッセージルール（リリース通知用）

`master` への push 時に Slack へリリース情報を自動送信するため、
コミットメッセージは以下のフォーマットで記載すること。

### フォーマット

```
ver. X.Y.Z: [概要（1行）]

[add]
- 追加・変更した機能の説明（1件1行）

[fix]
- 修正したバグの説明（1件1行）

[change]
- その他変更（リファクタ・設定変更など）
```

### セクション定義

| セクション | 意味 | 記載が必要なとき |
|---|---|---|
| `[add]` | 機能追加 | 新しい機能・ページを追加したとき |
| `[fix]` | バグ修正 | 不具合を修正したとき |
| `[change]` | その他変更 | リファクタ・設定変更など（任意） |

- `[add]` / `[fix]` の両方が不要な場合（例：ドキュメントのみの変更）はセクションを省略してよい
- 箇条書きは体言止めで書く（例：「検索フィルターを追加」「ログイン時のクラッシュを修正」）

### デプロイ前の確認義務（Claude の責任）

`master` への push を行う前に、必ず以下を確認すること。

1. 直近の変更内容に対して、コミットメッセージに `[add]` / `[fix]` が適切に記載されているか確認する
2. 記載が不足している場合は **デプロイ前に** オーナーへ以下を伝える：
   - どのセクションの情報が足りないか
   - 何を書けばよいか（具体例を添えて）
3. オーナーが情報を補足・承認してからデプロイを進める

不足時の伝え方の例：

```
🟡(●▽●)ヌルフフ  デプロイ前チェック！
今回の変更を確認したところ、コミットメッセージに情報が不足しています。

- [add] セクション：○○機能を追加しましたが説明が未記載です
  → 例）「モンスター図鑑に属性フィルターを追加」

補足いただけますか？ヌルフフ
```
